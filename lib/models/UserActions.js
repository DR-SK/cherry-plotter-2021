...
module.exports = {
  async createUserActions(gameId) {
    // Known bug: room_id is hard-coded here
    ...
    // missing npc data
    return _.uniq(_.concat(npcs, items));
  },
  ...
  async performAction(action, target, gameId, userId) {
    if (action === "pick up") {
      ...
      // Check if the npc's HP is 0 or above
      // If true, they immediately attack back (using a dice roll against their base_atk)
      if (npcHP > 0) {
      ...
      if (playerHack >= npcHack) {
        ...
        // Remove all actions from this npc, it's disabled
        await pool.query(
          `
          DELETE FROM game_npcs
          WHERE npc_id = $1
      ...
      // hard-coding the room_id for now
