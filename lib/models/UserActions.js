...
module.exports = {
  ...
  async performAction(action, target, gameId, userId) {
    if (action === "pick up") {
      ...
      let { rows: attack } = await pool.query(
        `
      SELECT 
        base_atk
      FROM 
      ...
      attack = roll([[attack[0].base_atk, 6]]);

      // get the target's HP
      const { rows: targetHP } = await pool.query(
        `
        SELECT 
          hp
        FROM 
      ...
      // if the npc's HP is 0 or below, remove attack from the list of npc actions on that npc.npc_id
